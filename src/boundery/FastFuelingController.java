...
public class FastFuelingController implements Initializable {
	...
	void procedPayment(ActionEvent event) {
		...
		FuelPurchase purchase = new FuelPurchase(null, stationID, car.getCarNumber(), amount, customerPrice * amount,
				...
				companiesCombo.getValue(), car.getFuelType());

		if (visaRadio.isSelected())
			result = CustomerCC.commitFuelPurchase(customer.getId(), customer.getPricingModel(),
					customer.getVisaNumber(), purchase, car.getFuelType());

		else if (cashRadio.isSelected())
			result = CustomerCC.commitFuelPurchase(customer.getId(), customer.getPricingModel(), "CASH", purchase,
					car.getFuelType());
		else {
			if (customer.getPricingModel() == 3)
				result = CustomerCC.commitFuelPurchase(customer.getId(), customer.getPricingModel(), null, purchase,
						car.getFuelType());
			else {
	...
	void nextToPayment(ActionEvent event) {
		...
		ArrayList<Float> resDetails = CustomerCC.getPurchasePriceDetails(companyName, customer.getId(),
				car.getFuelType(), customer.getPricingModel(), pubmpNumberCombo.getValue());
